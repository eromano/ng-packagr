...
export const transformSourcesTransform: Transform = pipe(
  map(graph => {
    ...
    const inlineResources = componentTransformer({
      template: ({ templateFilePath }) => graph.find(node => node.url === `file://${templateFilePath}`).data,
      stylesheet: ({ styleFilePath }) => graph.find(node => node.url === `file://${styleFilePath}`).data
    });
