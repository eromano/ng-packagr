...
export const stylesheetTransform: Transform = pipe(
  map(graph => {
    ...
    stylesheetNodes.forEach(async stylesheetNode => {
      const filePath: string = stylesheetNode.data.filePath;
      ...
      stylesheetNode.data.content = result;
    });
